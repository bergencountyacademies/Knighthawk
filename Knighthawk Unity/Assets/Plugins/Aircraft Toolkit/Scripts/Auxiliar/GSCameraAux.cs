...
public class GSCameraAux: MonoBehaviour {
	...
	public enum ActivationCondition { never, ifUnity4, ifUnity5, always }

	GAircraft sm = null;
	public Camera attachToCamera = null;
	GUITexture crossair = null;
	...
	public ActivationCondition fogEnabledIf = ActivationCondition.ifUnity4;
	...
	public ActivationCondition skyboxRenderingEnabledIf = ActivationCondition.ifUnity4;
	public bool skyboxRenderingSunEnabled = true;
	...
	public bool skyboxApplyToSeaReflection = true;
	private Cubemap skyboxReflectionCubemap = null;
	[System.NonSerializedAttribute]private Color[] skyboxReflectionCubemapPixels = null;
	[System.NonSerializedAttribute]private int skyboxReflectionCubemapSize = -1;

	public bool searchSounds = true;
	[HideInInspector]public GUIText speedmeter;
	[HideInInspector]public GUIText autopilotmeter;
	...
	void Skybox_Start() {
		...
		if (skyboxComponent == null) ((attachToCamera == null) ? Camera.main : attachToCamera).gameObject.AddComponent<Skybox>();
		if (skyboxComponent == null) skyboxComponent = (Skybox)((attachToCamera == null) ? Camera.main : attachToCamera).gameObject.GetComponent("Skybox");
		...
		if (skyboxApplyToSeaReflection) {
			if ((ground != null) && (ground.GetComponent<Renderer>() != null) && (ground.GetComponent<Renderer>().material != null)) {
				skyboxReflectionCubemapSize = (skyboxTextureWidth > skyboxTextureHeight) ? skyboxTextureWidth : skyboxTextureHeight;
				skyboxReflectionCubemap = new Cubemap(skyboxReflectionCubemapSize, TextureFormat.ARGB32, false);
				ground.GetComponent<Renderer>().material.SetTexture("_Cube", skyboxReflectionCubemap);
				skyboxReflectionCubemapPixels = new Color[skyboxReflectionCubemapSize * skyboxReflectionCubemapSize];
			}
		}
	}

	void Start() {
		if (GameObject.Find(searchScreenCursorObjectName) != null) crossair = (GUITexture)((GameObject.Find(searchScreenCursorObjectName)).GetComponent("GUITexture"));
		if (GameObject.Find(searchScreenSpeedMeterObjectName) != null) speedmeter = (GUIText)((GameObject.Find(searchScreenSpeedMeterObjectName)).GetComponent("GUIText"));
		if (GameObject.Find(searchScreenAutopilotMeterObjectName) != null) autopilotmeter = (GUIText)((GameObject.Find(searchScreenAutopilotMeterObjectName)).GetComponent("GUIText"));
		...
		#if UNITY_5
			if ((fogEnabledIf == ActivationCondition.ifUnity5) || (fogEnabledIf == ActivationCondition.always)) {
				//fogEnabled = fogEnabled;
			} else {
				fogEnabled = false;
			}
			if ((skyboxRenderingEnabledIf == ActivationCondition.ifUnity5) || (skyboxRenderingEnabledIf == ActivationCondition.always)) {
				//skyboxRenderingEnabled = skyboxRenderingEnabled;
			} else {
				skyboxRenderingEnabled = false;
			}
		#elif UNITY_4
			if ((fogEnabledIf == ActivationCondition.ifUnity4) || (fogEnabledIf == ActivationCondition.always)) {
				//fogEnabled = fogEnabled;
			} else {
				fogEnabled = false;
			}
			if ((skyboxRenderingEnabledIf == ActivationCondition.ifUnity4) || (skyboxRenderingEnabledIf == ActivationCondition.always)) {
				//skyboxRenderingEnabled = skyboxRenderingEnabled;
			} else {
				skyboxRenderingEnabled = false;
			}
	...
	bool PreProcessSkyboxTexture(float height) {
		...
		if (skyboxRenderingColor0Enabled) {
			...
			hm1 = Mathf.RoundToInt(skyboxAngleColors * ((ahm1 + Mathf.PI / 2.0f) / Mathf.PI));
		}
	...
	bool ProcessSkyboxTexture(CubemapFace faceid, Texture2D tex, float height, int mode) {
		if (pixels == null) Skybox_Start();
		int w, h;
		w = tex.width;
		h = tex.height;
		int wc, hc;
		if (faceid == CubemapFace.PositiveY) {
			wc = Mathf.FloorToInt(w * 0.3f);
			hc = Mathf.FloorToInt(h * 0.5f);
		} else {
			wc = Mathf.FloorToInt(w * 0.5f);
			hc = Mathf.FloorToInt(h * 0.9f);
		}
		for (int j = 0; j < h; ++j) {
			switch (mode) {
				case -1:
				...
				case 1:
					for (int i = 0; i < w; ++i) {
						...
						if (skyboxRenderingSunEnabled && (faceid == CubemapFace.PositiveY)) {
							int a = ((i - wc) * (i - wc) / 3 + (j - hc) * (j - hc)) * 2;
							a = a * a / 35;
							if (a > 255) a = 0;
							else a = 255 - a;
							if (a > pixels[i + j * w].r) pixels[i + j * w].r = (byte)a;
							if (a > pixels[i + j * w].g) pixels[i + j * w].g = (byte)a;
							if (a > pixels[i + j * w].b) pixels[i + j * w].b = (byte)a;
							if (a > pixels[i + j * w].a) pixels[i + j * w].a = (byte)a;
						}
					}
					break;
			}
		...
		tex.Apply(false);
		if (skyboxApplyToSeaReflection && (skyboxReflectionCubemap != null)) {
			int ii, jj;
			for (int j = 0; j < skyboxReflectionCubemapSize; ++j) {
				for (int i = 0; i < skyboxReflectionCubemapSize; ++i) {
					ii = i * w / skyboxReflectionCubemapSize;
					jj = j * h / skyboxReflectionCubemapSize;
					skyboxReflectionCubemapPixels[i + j * w] = pixels[ii + (h - jj - 1) * w];
				}
			}
			skyboxReflectionCubemap.SetPixels(skyboxReflectionCubemapPixels, faceid);
			skyboxReflectionCubemap.Apply(false);
		}
		return true;
	}
	...
	void ProcessSkybox() {
		...
		if (fogEnabled && fogRenderingEnabled) {
			...
			if (skyboxRenderNth_count <= 0) {
				...
				for (int i = 0; i < skyboxRenderSidesPerNth; ++i) {
					...
					switch (skyboxRenderNth_tex) {
						case 0:
							if (_FrontTex != null) ProcessSkyboxTexture(CubemapFace.PositiveZ, _FrontTex, gameObject.transform.position.y + smheight, 0);
							break;
						case 1:
							if (_LeftTex != null) ProcessSkyboxTexture(CubemapFace.PositiveX, _LeftTex, gameObject.transform.position.y + smheight, 0);
							break;
						case 2:
							if (_RightTex != null) ProcessSkyboxTexture(CubemapFace.NegativeX, _RightTex, gameObject.transform.position.y + smheight, 0);
							break;
						case 3:
							if (_BackTex != null) ProcessSkyboxTexture(CubemapFace.NegativeZ, _BackTex, gameObject.transform.position.y + smheight, 0);
							break;
						case 4:
							if (_UpTex != null) ProcessSkyboxTexture(CubemapFace.PositiveY, _UpTex, gameObject.transform.position.y + smheight, 1);
							break;
						case 5:
							if (_DownTex != null) ProcessSkyboxTexture(CubemapFace.NegativeY, _DownTex, gameObject.transform.position.y + smheight, -1);
							break;
					}
				}
			}
	...
	void ProcessCamera(float timeLapsus) {
		...
		if (cameraExternalEnabled) {
			...
			if (cameraExternalDistance < cameraExternalDistanceMin) cameraExternalDistance = cameraExternalDistanceMin;
			if (cameraExternalDistance > cameraExternalDistanceMax) cameraExternalDistance = cameraExternalDistanceMax;
