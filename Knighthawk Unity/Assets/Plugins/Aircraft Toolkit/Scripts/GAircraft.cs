...
[RequireComponent(typeof(Rigidbody))]public class GAircraft: MonoBehaviour {
	...
	public KeyCode inputThrottleKeyForEngineToggle = KeyCode.F4;
	...
	[HideInInspector]public bool inputBrakes_internal_enabled = false;
	...
	[HideInInspector]public float inputTrails_output = 0.0f;
	...
	public bool initializeLineRenderers = true;
	private bool initializeLineRenderersHold = false;
			...
			if (ch.activeSelf) {
				...
				if (sf != null) {
					...
					if (surfaces_count < surfaces_maxcount) {
						...
						if (initializeLineRenderersHold && (ch.GetComponent("LineRenderer") == null)) ch.AddComponent<LineRenderer>();
						if (initializeLineRenderersHold) surfaces[surfaces_count].lineRenderer = (LineRenderer)ch.GetComponent("LineRenderer");
						else surfaces[surfaces_count].lineRenderer = null;
						++surfaces_count;
					}
					log(ch.name + " has GSurface");
					if (!globalRenderPhysicSurfaces) { if (ch.GetComponent<Renderer>() != null) ch.GetComponent<Renderer>().enabled = false; }
					...
					if (labeled_surfaces_count < labeled_surfaces_maxcount) {
						...
						labeled_surfaces[labeled_surfaces_count].shapeXNegative = GSurface.shapeTypeFromParameter(labeled_surfaces[labeled_surfaces_count].shapeXNegativeParameter, labeled_surfaces[labeled_surfaces_count].shapeXNegative);
						labeled_surfaces[labeled_surfaces_count].shapeYPositive = GSurface.shapeTypeFromParameter(labeled_surfaces[labeled_surfaces_count].shapeYPositiveParameter, labeled_surfaces[labeled_surfaces_count].shapeYPositive);
						labeled_surfaces[labeled_surfaces_count].shapeYNegative = GSurface.shapeTypeFromParameter(labeled_surfaces[labeled_surfaces_count].shapeYNegativeParameter, labeled_surfaces[labeled_surfaces_count].shapeYNegative);
						labeled_surfaces[labeled_surfaces_count].shapeZPositive = GSurface.shapeTypeFromParameter(labeled_surfaces[labeled_surfaces_count].shapeZPositiveParameter, labeled_surfaces[labeled_surfaces_count].shapeZPositive);
						labeled_surfaces[labeled_surfaces_count].shapeZNegative = GSurface.shapeTypeFromParameter(labeled_surfaces[labeled_surfaces_count].shapeZNegativeParameter, labeled_surfaces[labeled_surfaces_count].shapeZNegative);
						log(
							"  " + ch.name + " shape is" +
						...
						if (initializeLineRenderersHold && (ch.GetComponent("LineRenderer") == null)) ch.AddComponent<LineRenderer>();
						if (initializeLineRenderersHold) labeled_surfaces[labeled_surfaces_count].lineRenderer = (LineRenderer)ch.GetComponent("LineRenderer");
						else labeled_surfaces[labeled_surfaces_count].lineRenderer = null;
					...
					if (ch.name.Contains("_hide_")) { if (ch.GetComponent<Renderer>() != null) ch.GetComponent<Renderer>().enabled = false; }
					else if (!globalRenderPhysicSurfaces) { if (ch.GetComponent<Renderer>() != null) ch.GetComponent<Renderer>().enabled = false; }
				} else if (ch.name.Contains("_hide_")) {
					if (ch.GetComponent<Renderer>() != null) ch.GetComponent<Renderer>().enabled = false;
				}
				if (sp != null) {
				...
				if (sd != null) {
					if (surfacedrives_count < surfacedrives_maxcount) {
						...
						if (initializeLineRenderersHold && (ch.GetComponent("LineRenderer") == null)) ch.AddComponent<LineRenderer>();
						if (initializeLineRenderersHold) surfacedrives[surfacedrives_count].lineRenderer = (LineRenderer)ch.GetComponent("LineRenderer");
						else surfacedrives[surfacedrives_count].lineRenderer = null;
						++surfacedrives_count;
					}
					...
					if (labeled_surfacedrives_count < labeled_surfacedrives_maxcount) {
						...
						if (initializeLineRenderersHold && (ch.GetComponent("LineRenderer") == null)) ch.AddComponent<LineRenderer>();
						if (initializeLineRenderersHold) labeled_surfacedrives[labeled_surfacedrives_count].lineRenderer = (LineRenderer)ch.GetComponent("LineRenderer");
						else labeled_surfacedrives[labeled_surfacedrives_count].lineRenderer = null;
		...
		for (int i = 0; i < e.transform.childCount; ++i) {
			...
			if (ch.activeSelf) {
				...
				if (st != null) {
					...
					if (initializeLineRenderersHold) {
	...
	void ProcessDrives() {
		...
		bool poweredToggle = Input.GetKeyDown(inputThrottleKeyForEngineToggle);
		...
		for (int i = 0; i < surfacedrives_count; ++i) {
			...
			if (poweredToggle) surfacedrives[i].powered = !surfacedrives[i].powered;
		...
		for (int i = 0; i < labeled_surfacedrives_count; ++i) {
			...
			if (poweredToggle) labeled_surfacedrives[i].powered = !labeled_surfacedrives[i].powered;
	...
	void Start() {
		initializeLineRenderersHold = initializeLineRenderers;
	...
	public int simulationFirstFrames = 1;
